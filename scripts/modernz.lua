...
layouts["modern-compact"] = function()
    ...
    lo = add_layout("title")
    lo.geometry = geo
    lo.style = string.format("%s{\\clip(%f,%f,%f,%f)}", osc_styles.title, geo.x, geo.y - geo.h, geo.x + geo.w,
        geo.y + geo.h)
    lo.alpha[3] = 0
    ...
    if user_opts.show_chapter_title then
        ...
        lo.style = string.format("%s{\\clip(%f,%f,%f,%f)}", osc_styles.chapter_title, chapter_geo.x,
            chapter_geo.y - chapter_geo.h, chapter_geo.x + chapter_geo.w, chapter_geo.y + chapter_geo.h)
    end
    -- Time codes
    lo = add_layout("time_codes")
...
local function osc_init()
    ...
    ne.tooltipF = function()
        return user_opts.tooltip_hints and
            (state.file_loop and locale.file_loop_disable or locale.file_loop_enable) or ""
    end
    ne.eventresponder["mbtn_left_up"] = function()
        mp.command("show-text '" .. (state.file_loop and locale.file_loop_disable or locale.file_loop_enable) .. "'")
        state.file_loop = not state.file_loop
